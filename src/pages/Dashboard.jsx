import { LoaderCircleIcon, PlusIcon, UploadCloud, UploadCloudIcon, XIcon } from 'lucide-react'
...
const Dashboard = () => {
  ...
  useEffect(()=>{
    ...
    const resumeData = dummyResumeData.map(resume => ({
      _id: resume._id,
      title: resume.title,
      updatedAt: new Date().toISOString() // Use current date for demo
    }))
    setAllResumes(resumeData)
    // loadAllResumes() // Commented out for now
  },[])
      ...
      <div className="grid grid-cols-2 sm:flex flex-wrap gap-4 ">
        {allResumes.map((resume, index) => (
          <ResumeCard 
            ...
            index={index}
            colors={colors}
            onEdit={(resumeId, title) => {setEditResumeId(resumeId); setTitle(title)}}
